    ...
    >
      ...
      <div className="relative z-10 p-4">
            ...
            <div key={group.title} className="mb-6">
              ...
              >
                <span className="flex items-center gap-2">
                  <div className="w-1 h-4 bg-linear-to-b from-cyan-400 to-purple-400 rounded-full group-hover:h-5 transition-all" />
                  {group.title}
                </span>
                <ChevronDown className={`w-4 h-4 transition-transform duration-300 ${isOpen ? "rotate-180" : "rotate-0"}`} />
              </button>

              <div className={`space-y-2 transition-all duration-300 ${isOpen ? 'opacity-100 max-h-250' : 'opacity-0 max-h-0 overflow-hidden'}`}>
                  ...
                  return (
                    <div
                      ...
                      className={`group relative overflow-hidden rounded-lg border transition-all duration-300 ${
                        active 
                          ? "bg-linear-to-r from-cyan-500/20 to-purple-500/20 border-cyan-400/40 shadow-lg shadow-cyan-500/10" 
                          : "bg-black/40 border-white/10 hover:border-cyan-400/30 hover:bg-black/60"
                      }`}
                    >
                      {/* Active indicator glow */}
                      {active && (
                        <div className="absolute inset-0 bg-linear-to-rrom-cyan-400/10 to-purple-400/10 blur-xl" />
                      )}
              ...
              {group.title === "CODES — NAVIGATION" && isOpen && (
                <div className="mt-6 rounded-xl border border-cyan-500/20 bg-linear-to-brrom-cyan-950/20 via-black/60 to-purple-950/20 backdrop-blur-xl p-4 shadow-lg">
        ...
        <div className="mt-8 pt-4 border-t border-cyan-500/20">
          <div className="bg-linear-to-r from-cyan-500/5 to-purple-500/5 rounded-lg p-3 border border-cyan-500/10">
